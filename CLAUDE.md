...
> **Last updated:** 2026-04-24 (v5.27 — demo-prep-1 F&F reliability pack: `/recommend` 422 → explicit `RecommendationState.insufficientData`; `UnresolvedProductView` for 404s; low-confidence (<0.70) 409 gate on `/resolve-from-search` + `/confirm` endpoint + `ConfirmationPromptView` sheet; `make demo-check` + `make demo-warm` CLIs. Backend 597→609, iOS 179→190.)
...
**Test totals:** 609 backend + 190 iOS unit + 6 iOS UI (with experiment flags off — see L-Experiment-flags-default-off). `ruff check` clean. `xcodebuild` clean.
...
2. **Phase 3:** 3a–3d-noise-filter ✅ (#22–#36), ui-refresh-v1/v2/v2-fix ✅ (#37–#40), 3e (#41), 3f (+hotfix) ✅ (#42, #44), Benefits Expansion ✅ (#45–#47), FB Marketplace location + resolver ✅ (#48, #49), experiment/tier2-ebay-search ✅ (#50, opt-in), fb-resolver-followups + postfix-1 ✅ (#51, #52), 3g-A + 3g-B + 3g-B-fix-1 ✅ (#53, #54, #55), search-resolve-perf-1 ✅ (#61), search-relevance-1 ✅ (#62), demo-prep-1 ✅ (#63, AppIcon drop-in deferred on Figma). Next: pack 2 (savings-math-prominence), AppIcon PNGs when Figma lands, prod FB seed (Mike), eBay-Tier-2 graduation call, 3h Vision, 3i receipts, 3k savings, 3l coupons, 3m hardening + `v0.3.0`. 3j folded into 3e
...
- **demo-prep-1 (#63).** Explicit states over silent-nil. `/recommend` 422 → `RecommendationState.insufficientData(reason:)` w/ dedicated card; pre-existing FastAPI envelope `{detail:{error:...}}` decode fix in `APIClient.decodeErrorDetail` (messages had been lost on the floor for every endpoint). `UnresolvedProductView` for `/resolve` + `/resolve-from-search` 404s routed through both Scanner + Search via new `TabSelectionAction` env (also unblocks pill→Profile cross-tab nav). Low-confidence 409 gate on `/resolve-from-search` at `LOW_CONFIDENCE_THRESHOLD=0.70` (env-tunable) — gate fires BEFORE Gemini so rejection has zero AI-credit cost. `POST /resolve-from-search/confirm` commits + marks `Product.source_raw.user_confirmed=True`; `user_confirmed=false` logs only. `make demo-check` + `make demo-warm` CLIs w/ first repo-root Makefile. AppIcon deferred on Figma handoff
