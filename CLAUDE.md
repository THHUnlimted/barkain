...
**Test totals:** **335 + 6 + 29 + 26 + 42 + 10 + 14 + 26 backend** (3c-hardening adds 26 across `test_m2_prices.py` accessory + platform-suffix, `test_walmart_http_adapter.py` retry/back-off, `test_best_buy_api.py` 429/5xx + query sanitizer, `test_product_resolve_from_search.py` device→UPC cache, `test_m2_prices_stream.py` scoped query cache — re-sum after your next full run) + **77 iOS unit** + **3 iOS UI**.
...
2. **Phase 3 — Recommendation Intelligence (IN PROGRESS):** 3a ✅ text search (#22, #23). 3b ✅ eBay Browse API + deletion webhook (#24). 3c ✅ Search v2 (cascade + variant collapse + deep search + eBay affiliate fix, PR #32). 3c-hardening ✅ Live-test follow-on bundle: Amazon platform-suffix accessory filter, service/repair listing filter, Walmart 5× retry + back-off, Best Buy 429/5xx retry, Best Buy query sanitizer, Redis device→UPC cache, scoped query_override cache, iOS sheet-anchoring fix (PR #32). Next: 3d M6 Recommendation Engine (Claude Sonnet synthesis), 3e card rewards, 3f portal stacking, 3g image scan, 3h receipts, 3i identity stacking, 3j savings dashboard, 3k coupons, 3l hardening + `v0.3.0`.
