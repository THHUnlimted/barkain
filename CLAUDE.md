...
> **Last updated:** 2026-04-27 (v5.38 — bench/vendor-migrate-1: production AI-resolve leg now Serper-then-grounded (E-then-B). New `backend/ai/web_search.py:resolve_via_serper`; `gemini_generate` gains `grounded`/`thinking_budget` kwargs; long-standing temperature=1.0 hardcode fixed. Bench (Mike-verified 9 UPCs): 100% recall vs grounded's 53%; p50 -47%; ~36× cheaper. Backend 694→711. Opens `vendor-migrate-1-L1` (Serper coverage tail). Full history in `docs/CHANGELOG.md`.)
