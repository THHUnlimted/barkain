...
> **Last updated:** 2026-04-29 (v5.41 — PR #81 made `isMiscRetailerEnabled` Debug-default-ON / Release-default-OFF (standard-suite-gated so test suites still default OFF). Canary lever is now server-side `MISC_RETAILER_ADAPTER` only. Live-validated on Mike's iPhone 15. Per-step detail + per-PR rationale in `docs/CHANGELOG.md`.)
