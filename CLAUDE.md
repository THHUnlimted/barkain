...
> **Last updated:** 2026-04-29 (v5.48 — `thumbnail-coverage-L1` close + `SP-L1-b` retired. **`thumbnail-coverage-L1`** (code, iOS) — `SearchResultRow` placeholder upgraded from generic `shippingbox` SF Symbol to a brand-aware tile: when `result.brand` parses to ≥1 letter the tile renders 1–2 brand initials in white on the warm-gold `barkainPrimaryGradient` (`Apple Inc` → "AI", `Rust-Oleum` → "RO", `3M Company` → "C", `Apple` → "AP", lowercase upcased). Falls back to a `pawprint.fill` glyph on `barkainPrimaryFixed` when no usable brand exists (`(generic)` / `123` / `—` / nil). New static `SearchResultRow.brandInitials(from:)` helper — Unicode-aware via `Character.isLetter`, splits on whitespace + hyphen, multi-token takes first letter of first ≤2 letter-leading tokens, single-token requires letter-led and takes first ≤2 letters. +8 unit tests in new `BarkainTests/Features/Search/SearchResultRowTests.swift`. Originally deferred for "miss-rate telemetry"; closed early because the fix is purely a cosmetic placeholder and adds no upstream cost. **`SP-L1-b`** (HIGH) — token revoked by Mike in GitHub UI; scrubbed from CLAUDE.md Known Issues + What's Next + recently-closed marker. Historical references in `docs/CHANGELOG.md` 2i-d entry + `docs/PHASES.md` 2i-d row updated to mark as **resolved**. Known Issues 3 → 1 open (only `2i-d-L4` MED remains; that needs the new container `/page-html` endpoint design and was explicitly deferred this session). Original v5.47 detail preserved below. — Known-Issues triage rounds 1+2+3: 15 → 3 open. Round 3 (this turn) **closes all five remaining LOWs** with two code fixes + three documented closures. **`thumbnail-coverage-L3`** (code) — `_KNOWN_BAD_IMAGE_HOSTS = ("demandware.net",)` blocklist + `_filter_known_bad_image_url(url)` helper at the top of `m1_product/service.py`. Hosts in this set (suffix-matched on netloc) write `image_url=NULL` at persist time so the scraper-backfill stays eligible AND the iOS fallback chain becomes canonical. Soft-fails on parse error. **`vendor-migrate-1-L1`** (code) — Serper outcome counter at `metrics:serper_resolve:outcomes` Redis hash, four buckets (`success`/`synthesis_null`/`serper_miss`/`synthesis_error`). New `_record_serper_outcome` helper in `ai/web_search.py` lazy-creates an aioredis client per call, soft-fails on any Redis error so telemetry never blocks the resolve hot path. Ops can `HGETALL` once and divide for the exact fallback ratio. **`2i-d-L3`** (closed stale) — `ebay_new` / `walmart` no longer use container scrapers (`ebay_browse_api` + `walmart_http`), so selector_drift is structurally impossible. **`thumbnail-coverage-L2`** (closed won't-fix) — `RecentlyScannedStore` self-heals on re-tap; explicit-defer reaffirmed. **`misc-retailer-L1`** (closed ops-only) — `make bench-misc-retailer` already exists; weekly cron lives outside repo. Backend tests +10 (789→799 passed; 8 skipped). 5 thumbnail tests in `tests/modules/test_m1_product.py` (3 unit-level filter + 2 integration end-to-end persistence). 5 Serper outcome tests in new `tests/ai/test_web_search_outcomes.py` covering all four buckets + Redis-soft-fail. Known Issues 15 → 3 open; HIGH still open (SP-L1-b, GitHub UI revoke for Mike); 1 MEDIUM (2i-d-L4, deferred per session scope — needs new container `/page-html` endpoint design); 1 LOW (`thumbnail-coverage-L1`, explicitly deferred until miss-rate telemetry is in place). Round 1+2 detail (originally v5.46): 15 → 4 open. Round 1 closed `noise-filter-L1` (`"game download"` in `_TIER2_HARD_NOISE_CATEGORY_TOKENS`), `3o-C-L1-fabricated-upc-tap` + `rustoleum-ux-L1` + `rustoleum-ux-L2` (via commit `e1236da`), and `v4.0-L2` (superseded by `m2_prices/service.py:551` Rule 2b ordinal gate). Round 2 closed two more: **`2h-ops`** — SQS DLQ wiring (`workers/queue_client.py` adds `DLQ_SUFFIX`/`DLQ_MAX_RECEIVE_COUNT`/`dlq_name` + `SQSClient.create_queue_with_dlq` + `set_queue_attributes` + `get_queue_arn`; `scripts/run_worker.py setup-queues` now provisions sibling `<name>-dlq` queues with `RedrivePolicy{maxReceiveCount=3}`. Idempotent. Validated end-to-end against LocalStack — 6 queues + redrive-policy verified. +4 backend tests). **`cat-rel-1-L2-ux`** — Gemini reasoning surfaces in `/resolve-from-search` 404s. Backend: `_lookup_upc_from_description` returns `tuple[upc, reasoning]`; `UPCNotFoundForDescriptionError(reasoning=)`; router lifts it to `details.reasoning`. iOS: `APIError.notFound` → `notFound(reason: String? = nil)`; `apiErrorFor` + raw 404 path extract; `SearchViewModel.unresolvedReason` + `OptimisticResolveOutcome.unresolved(reason:)` + `UnresolvedProductView` italicized reason display. ScannerView migrated `error == .notFound` → `case .notFound = error` pattern. +2 backend tests pinning reasoning-surfaces / reasoning-omitted-on-silent-Gemini. Test files updated to `.notFound()` construction across 4 BarkainTests classes. Backend tests +6 (784→790 passed; 8 skipped). Known Issues 15 → 8 open; HIGH still open (SP-L1-b, GitHub UI revoke for Mike); 1 MEDIUM (2i-d-L4, deferred per session scope — needs new container `/page-html` endpoint design); 6 LOWs all monitoring/deferred/self-mitigated (2i-d-L3 health-data, vendor-migrate-1-L1 telemetry, 4× thumbnail/misc-retailer ops items). Per-step detail in `docs/CHANGELOG.md`.)
...
| known-issues-triage-3 | Final LOW sweep — five closes (2 code, 3 documented). **thumbnail-coverage-L3** (code) — `_KNOWN_BAD_IMAGE_HOSTS = ("demandware.net",)` blocklist + `_filter_known_bad_image_url(url)` helper at the top of `m1_product/service.py`; suffix-matches netloc (so `images.salsify.com.demandware.net` also drops); soft-fails on parse error. Wired into `_persist_product` so blocklisted hosts produce `image_url=NULL` at insert-time, preserving scraper-backfill eligibility AND letting the iOS `fallbackImageUrl` chain take over for those rows. **vendor-migrate-1-L1** (code) — Serper outcome counter at `metrics:serper_resolve:outcomes` Redis hash. `_record_serper_outcome(bucket)` increments one of four buckets per `resolve_via_serper` call: `success` / `synthesis_null` / `serper_miss` / `synthesis_error`. Lazy aioredis client per call, soft-fails on Redis errors. Ops `HGETALL` for exact fallback ratio. Closes the "watch log frequency" follow-up with a structured metric. **2i-d-L3** (closed stale) — `ebay_new` / `walmart` no longer use container scrapers (now `ebay_browse_api` + `walmart_http`), so selector_drift is structurally impossible for them. **thumbnail-coverage-L2** (closed won't-fix) — `RecentlyScannedStore` self-heals on re-tap. **misc-retailer-L1** (closed ops-only) — `make bench-misc-retailer` already exists; cron lives outside this repo. +10 backend tests: 5 thumbnail (3 unit-level filter + 2 integration end-to-end persistence in `tests/modules/test_m1_product.py`); 5 Serper outcome (all four buckets + Redis-soft-fail in new `tests/ai/test_web_search_outcomes.py`). Known Issues 8→3 open | +10 BE/+0 iOS | 0 | TBD |
| known-issues-triage-4 | `thumbnail-coverage-L1` close + `SP-L1-b` retired. **thumbnail-coverage-L1** (code, iOS) — `SearchResultRow` placeholder upgraded from `shippingbox` SF Symbol to a brand-aware tile. When `result.brand` parses to ≥1 alphabetic letter the tile renders 1–2 brand initials in white on the warm-gold `barkainPrimaryGradient` (Apple Inc → "AI", Rust-Oleum → "RO", 3M Company → "C", Apple → "AP", lowercase upcased, Unicode-aware via `Character.isLetter`). Falls back to a `pawprint.fill` glyph on `barkainPrimaryFixed.opacity(0.45)` when no usable brand exists ((generic) / 123 / — / nil). New static `SearchResultRow.brandInitials(from:)` helper splits on whitespace + hyphen; multi-token takes first letter of first ≤2 letter-leading tokens, single-token requires letter-led + takes first ≤2 letters. +8 unit tests in new `BarkainTests/Features/Search/SearchResultRowTests.swift` covering all branches (multi-token / hyphen-split / single-token / empty / non-alpha / unicode / digit-leading-skip). **SP-L1-b** (HIGH) — Mike revoked the leaked PAT in GitHub UI 2026-04-29; scrubbed from CLAUDE.md Known Issues + What's Next; historical references in `docs/CHANGELOG.md` 2c + 2i-d entries and `docs/PHASES.md` 2i-d row updated to mark resolved. Known Issues 3→1 open | 0 BE | +69 iOS / +1 new test file | TBD |

**Test totals:** 799 backend passed + 8 skipped (808 collected) + 215 iOS unit + 6 iOS UI (with experiment flags off — see L-Experiment-flags-default-off). known-issues-triage-4 added +8 iOS unit tests (207→215 passed; 8 in new `BarkainTests/Features/Search/SearchResultRowTests.swift`). known-issues-triage-3 added +10 backend tests (789→799 passed; 5 thumbnail in `tests/modules/test_m1_product.py` + 5 Serper outcome in new `tests/ai/test_web_search_outcomes.py`). known-issues-triage-2 added +6 backend tests (784→790 passed; 4 DLQ structural + 2 reasoning envelope) and rewires +9 iOS lines across APIError/APIClient/SearchViewModel/OptimisticPriceVM/UnresolvedProductView/SearchView/ScannerView (existing `.notFound` references migrated to `.notFound()` across 4 test files). known-issues-triage-1 added +1 backend test (783→784 passed). step-3o-C-followups (commit `e1236da`) added +2 backend tests (781→783 passed) for the Tier 3 UPC strip. 3o-C added +8 backend tests (773→781 passed). 3o-B added +19 backend tests (754→773 passed). 3o-A swung the count by −3 collected. `ruff check` clean. `xcodebuild` clean.
...
> **Triage round 4 (2026-04-29, this turn):**
> `thumbnail-coverage-L1` — `SearchResultRow` placeholder now renders brand initials on the warm-gold gradient when `result.brand` parses, else a pawprint glyph on `barkainPrimaryFixed`. New `SearchResultRow.brandInitials(from:)` helper + 8 unit tests covering multi-token / hyphen-split / single-token / empty / non-alpha / unicode cases.
> `SP-L1-b` — leaked PAT revoked by Mike in GitHub UI (2026-04-29). Scrubbed from CLAUDE.md Known Issues + What's Next; historical references in `docs/CHANGELOG.md` + `docs/PHASES.md` annotated as resolved.
> **Triage round 3 (2026-04-29, this session — all-LOW sweep):**
> `thumbnail-coverage-L3` — `_KNOWN_BAD_IMAGE_HOSTS` blocklist (`demandware.net`) at `_persist_product`; rows from blocklisted hosts now write `image_url=NULL` so backfill stays eligible AND the iOS fallback chain becomes canonical. Helper `_filter_known_bad_image_url` suffix-matches netloc; soft-fails on parse error.
> `vendor-migrate-1-L1` — Serper outcome counter at `metrics:serper_resolve:outcomes` (Redis hash, four buckets: `success`/`synthesis_null`/`serper_miss`/`synthesis_error`). Lazy aioredis client per call, soft-fails on Redis errors. Ops `HGETALL` → exact fallback ratio without log grep.
> `2i-d-L3` — closed stale: `ebay_new` / `walmart` no longer use container scrapers (now `ebay_browse_api` + `walmart_http`), so selector_drift is structurally impossible.
> `thumbnail-coverage-L2` — closed won't-fix: `RecentlyScannedStore` self-heals on re-tap; bulk-reset overkill confirmed.
> `misc-retailer-L1` — closed ops-only: `make bench-misc-retailer` already exists; weekly cron lives outside this repo.
> **Triage round 2 (2026-04-29, prior in session):**
> `2h-ops` — SQS DLQ wiring shipped; `setup-queues` now provisions sibling DLQs with RedrivePolicy maxReceiveCount=3 (validated against LocalStack).
> `cat-rel-1-L2-ux` — Gemini reasoning surfaces in `/resolve-from-search` 404 envelope and `UnresolvedProductView` displays it under generic copy.
> **Triage round 1 (2026-04-29, prior in session):**
> `noise-filter-L1` — `"game download"` added to `_TIER2_HARD_NOISE_CATEGORY_TOKENS`; regression test `test_existing_game_download_still_drops` pins both singular/plural BBY category strings.
