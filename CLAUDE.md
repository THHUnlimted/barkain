...
| SP-samsclub-decodo | RESOLVED | `sams_club` was deterministic-failing at ~100 s with 0 listings — Akamai `/are-you-human/` gate fired from AWS datacenter IP (homepage OK, `/s/` search redirects). Fix: same Decodo-scoped pattern as fb_marketplace (proxy relay + 13 telemetry kill flags + proxy bypass list + `SAMS_CLUB_DISABLE_IMAGES=1` default + homepage warmup kept for session cookies). Post-fix: 3/3 listings, ~97 s avg, ~850 KB/extract through Decodo. `scripts/ec2_deploy.sh` now sources `/etc/barkain-scrapers.env` and injects `DECODO_PROXY_*` env for both retailers via `case` on retailer name. Regression-guarded in `test_sams_club_extract_flags.py` (26 asserts). See docs/SCRAPING_AGENT_ARCHITECTURE.md §C.12 | — (follow-up: `*.samsclubimages.com` bypass to drop ~700 KB/run image CDN traffic off Decodo) |
...
> - **sams_club uses the same Decodo-scoped pattern** (2026-04-18, SP-samsclub-decodo). Sam's Club `/s/` gate → `/are-you-human?url=...`; homepage loads fine. Same 13 telemetry flags, same bypass list, `SAMS_CLUB_DISABLE_IMAGES=1` default. Homepage warmup is load-bearing (session cookies). `scripts/ec2_deploy.sh` sources `/etc/barkain-scrapers.env` and injects `DECODO_PROXY_{USER,PASS}` for both `fb_marketplace` and `sams_club` via a `case` on retailer name. **Gotcha: use `cut -d= -f2-`, not `-f2`, when reading Decodo creds from `docker inspect` — the base64 password can end in `=` and `-f2` strips it silently (symptom: `CONNECT tunnel failed, response 407` on first deploy).** Regression-guarded by `test_sams_club_extract_flags.py` (26 asserts). Full rationale: docs/SCRAPING_AGENT_ARCHITECTURE.md §C.12
