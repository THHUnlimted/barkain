...
> **Last updated:** 2026-04-21 (v5.9 — 3f Purchase Interstitial + activation reminder; killed 6 pre-existing auth failures; migration 0008 adds affiliate_clicks.metadata)
...
| 3f | Purchase Interstitial + Activation Reminder. `PurchaseInterstitialSheet` from hero CTA + row taps (card headline + baseline-1% delta + conditional activation block). `POST /affiliate/click` gains `activation_skipped` telemetry (migration 0008 `affiliate_clicks.metadata` JSONB). M6 cache key `:c<sha1>:i<sha1>:v2` (card/identity bust). Pre-fixes: `BarePreviewAPIClient` + `scripts/_db_url.py` + `without_demo_mode` fixture (killed 6 pre-existing auth failures, 8-step carry-forward). Alternatives rail scroll-to | +4 | +9 | — |

**Test totals:** 506 backend (0 pre-existing failures) + 117 iOS unit + 6 iOS UI. `ruff check` clean. `xcodebuild` clean.

**Migrations:** 0001 (initial, 21 tables) → 0002 (price_history composite PK) → 0003 (is_government) → 0004 (card catalog unique index) → 0005 (portal bonus upsert + failure counter) → 0006 (`chk_subscription_tier` CHECK) → 0007 (pg_trgm extension + `idx_products_name_trgm` GIN index) → 0008 (`affiliate_clicks.metadata` JSONB for interstitial telemetry). Drift marker in `tests/conftest.py::_ensure_schema` now checks `affiliate_clicks.metadata`.
...
- 3c-hardening: Amazon platform-suffix filter, Walmart 5× CHALLENGE retry, BBY 429/5xx retry + `_sanitize_query`, Redis device→UPC 24h + scoped query cache 30 min, iOS `browserURL @Binding` anchoring
- 3d-noise-filter: `_is_tier2_noise(row, *, query=None)` — category+title denylist + brand+model relevance. Query tokens len≥3 must be strict majority; any model-code (digit+letter, 4+ chars) must appear verbatim
- ui-refresh-v2 iOS kick-out fix: `SearchViewModel.onQueryChange` only dismisses presented product on non-empty, actually-changed query — `.searchable` setter fires `""` on nav-bar hide
- **3e M6 Recommendation Engine — deterministic.** Zero LLM. `POST /api/v1/recommend` does `asyncio.gather` over Prices+Identity+Cards+Portals then pure Python (< 150 ms p95). `final_price = base - identity_savings` (card + portal are deferred rebates). Tie-break: `effective_cost = final - card - portal`, then condition (new>refurb>used), then well-known retailer. Brand-direct callout at `*_direct` retailers with ≥15 %. Excludes `is_active=False` OR `retailer_health.status != ok`. Cache TTL 15 min. iOS hero gated on 3 settle flags (`streamClosed` + `identityLoaded` + `cardsLoaded`). Fetch failure → `recommendation=nil`, never alerts. Portal-bonus demo seed `scripts/seed_portal_bonuses_demo.py`, replaced in 3g
- **3f Purchase Interstitial — deterministic.** Reuses 3e `Recommendation` + the VM's already-loaded `CardRecommendation` list. `PurchaseInterstitialSheet` presents from both hero CTA and retailer row taps via one `@State interstitialContext` on `PriceComparisonView`. Activation ack is optimistic + session-scoped (no persistence table). `POST /affiliate/click` gains `activation_skipped` in JSONB `metadata` (migration 0008). M6 cache key `:c<sha1(card_ids)>:i<sha1(identity_flags)>:v2`. Baseline 1% hardcoded. Alternatives rail: `proxy.scrollTo(retailerId)` + 400 ms pulse. `APIClientProtocol.getAffiliateURL` 3-arg extension preserves callers. Pre-fixes: `BarePreviewAPIClient` base class, `scripts/_db_url.py`, `without_demo_mode` fixture + respx BBY route (killed 6 auth failures from 8-step carry-forward)
