...
> **Last updated:** 2026-04-22 (v5.16 — experiment/tier2-ebay-search: opt-in flags swap M1 Tier 2 UPCitemdb→eBay Browse, optional `gtin` surfacing + `SKIP_UPC` shortcut, M2 partial-listing denylist (`box only` / `for parts` / `charger only`) on `ebay_browse_api`, eBay seller-noise title sanitizer)
...
| experiment/tier2-ebay-search | Opt-in flags only — defaults preserve current behavior. **M1 Tier 2 swap:** `SEARCH_TIER2_USE_EBAY` replaces UPCitemdb leg of `gather(BBY, …)` with eBay Browse keyword search; `_sanitize_ebay_title` strips emoji / markdown / "FREE SHIPPING" / smart quotes so `/resolve-from-search` has clean signal. `SEARCH_TIER2_EBAY_USE_GTIN` requests `fieldgroups=EXTENDED` and surfaces `item.gtin` as `primary_upc` (Browse rarely returns it on item_summary, kept for forward-compat); `SEARCH_TIER2_EBAY_SKIP_UPC` forces `primary_upc=None` so iOS skips `/resolve` and goes straight to `/resolve-from-search`. Schema unchanged — eBay rows ride the existing `"upcitemdb"` source slot. **M2 noise:** `M2_EBAY_DROP_PARTIAL_LISTINGS` + `_is_partial_listing` regex drops `box only` / `for parts` / `charger only` / `as-is` / `screen replacement` listings on the `ebay_browse_api` price stream (used MacBook category was the trigger) | — | — | TBD |
...
- **experiment/tier2-ebay-search.** Four env flags, all default off → revertable by unset (`git checkout` for full rollback). `SEARCH_TIER2_USE_EBAY` swaps UPCitemdb for the Browse `item_summary/search` endpoint as Tier 2's second leg; rows still tagged `source="upcitemdb"` to avoid widening the iOS `Literal` enum. `_sanitize_ebay_title` was added because seller-crafted titles (emoji, `**Apple**`, "FREE SHIPPING", smart quotes) made `/resolve-from-search` 404 → iOS "couldn't pull details" toast. **GTIN finding:** Browse `item_summary` returns `gtin=None` even with `fieldgroups=EXTENDED` (epid only); per-item `/item/{itemId}` enrichment too expensive for picker latency, so `USE_GTIN` is wired but practically a no-op until eBay surfaces gtins natively. `SKIP_UPC` is therefore the de-facto behavior — eBay rows always flow through `/resolve-from-search`, which is the source of the ~1–3 s spinner on tap (server-side UPC derivation + Gemini cross-val, not client-side). **M2 partial-listing filter** (`M2_EBAY_DROP_PARTIAL_LISTINGS` + `_is_partial_listing`) added separately — used MacBook results were dominated by box-only / for-parts listings that crashed the rec engine's "best deal" tie-break. Regex is intentionally specific ("box only", not "box") — false positives on real listings are worse than letting one box through. Containers reachable from local backend via SSH tunnel (`ssh -L 8081…8091 ubuntu@54.197.27.219`) since SG keeps scraper ports VPC-only
