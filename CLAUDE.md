...
> **Last updated:** 2026-04-28 (v5.39 — feat/thumbnail-coverage: end-to-end image plumbing pass. Backend now (a) extracts `imageUrl` from Serper organic results in `resolve_via_serper`, (b) preserves UPCitemdb's full `images[]` array on `source_raw`, (c) backfills `Product.image_url` from the first scraper that returns one in `stream_prices`/`get_prices`, (d) ships `image_url` per row + `product_image_url` on `done`/`PriceComparisonResponse`. iOS `PriceComparison`/`RetailerPrice`/`StreamSummary` decode the new fields, `ProductCard` accepts a `fallbackImageUrl` chain that promotes on AsyncImage failure (rescues hotlink-blocked CDN URLs like `demandware.net`), `PriceRow.retailerIcon` renders an AsyncImage on the per-retailer URL. Live-validated on Makita XFD10Z + InfantLY in sim. Backend tests still 711. Opens `thumbnail-coverage-L1`–`L3` (see Known Issues). Full history in `docs/CHANGELOG.md`.)
...
**Test totals:** 711 backend + 203 iOS unit + 6 iOS UI (with experiment flags off — see L-Experiment-flags-default-off). `ruff check` clean. `xcodebuild` clean.
