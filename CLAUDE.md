...
> **Last updated:** 2026-04-28 (v5.42 — Step 3o-A autocomplete vocab expansion. Dropped `is_electronics()` filter; default sweep now hits 6 Amazon scopes (`aps`, `electronics`, `grocery`, `pet-supplies`, `tools`, `beauty`) plus 2 probe-admitted extras (`automotive`, `office-products`); HPC scope probe-rejected. `--max-terms` 5K → 15K. Sweep parallelized across sources via `asyncio.gather`. Bundle: 4,448 → 15,000 terms / 128 KB → 470 KB. iOS smoke confirmed: `cat`/`dog` surface 8 pet rows; `iph` regression-clean. Output schema bumped to `version=2`. Per-step detail + per-PR rationale in `docs/CHANGELOG.md`.)
...
**Test totals:** 754 backend + 207 iOS unit + 6 iOS UI (with experiment flags off — see L-Experiment-flags-default-off). 3o-A swung the count by −3 collected (parametrize collapse: dropped 9 `test_electronics_filter` cases, added 6 new). `ruff check` clean. `xcodebuild` clean.
