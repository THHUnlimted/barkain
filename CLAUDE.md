...
> **Last updated:** 2026-04-22 (v5.17 — fb-resolver-followups: dedicated `fb_location_resolve` rate bucket (5/min hard cap, no pro multiplier), iOS picker retry + canonical-redirect "start over" affordances, "Using SF default" pill on fb_marketplace row when `fb_location_id` absent, `source` → `resolution_path` rename + engine-name collapse to `{cache, live, seed, unresolved, throttled}`, top-50 US metro PG seed (44 resolved / 6 tombstoned))
...
| fb-resolver-followups | Bundle of low-risk follow-ups. Dedicated `fb_location_resolve` rate bucket (5/min hard cap, no pro multiplier — protects shared Decodo + search-engine token budget). API DTO renames `source` → `resolution_path` and collapses engine names `{startpage, ddg, brave, user}` → `live` so iOS sees the stable `{cache, live, seed, unresolved, throttled}` enum (DB column unchanged for analytics). `location_default_used` flag on fb_marketplace price payload when no `fb_location_id` was supplied → iOS pill "Using SF default — set your city in Profile". Picker FSM gains `retry()` + 3-attempt cap + 429-aware copy + `dismissCanonicalRedirect()`. Top-50 US-metro PG seed against local Docker — 50/50 resolved (44 first-pass, 3 recovered on retry-after-Redis-flush, 3 hand-resolved via web search: Oakland 108363292521622, Raleigh 103879976317396, Seattle 110843418940484) | +4 | +9 | #51 |

**Test totals:** 568 backend + 156 iOS unit + 6 iOS UI. `ruff check` clean. `xcodebuild` clean.
...
2. **Phase 3:** 3a–3d + 3d-noise-filter ✅ (#32–#36), ui-refresh-v1/v2/v2-fix ✅ (#37–#40), 3e (#41), 3f (#42), Benefits Expansion (#45–#47), fb-marketplace-location ✅, fb-marketplace-location-resolver ✅ (#49), experiment/tier2-ebay-search ✅ (#50, opt-in flags), fb-resolver-followups ✅ (rate bucket + retry + SF-default pill + rename + local seed). Next: prod seed (Mike-operated), wire pill→Profile cross-tab nav, decide if eBay-as-Tier-2 graduates from experiment, 3g portal live scrape, 3h Claude Vision, 3i receipts, 3k savings, 3l coupons, 3m hardening + `v0.3.0`. 3j folded into 3e. See `docs/CHANGELOG.md` + `docs/PHASES.md`.
