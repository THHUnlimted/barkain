...
| 3d | Autocomplete (vocab + iOS integration): on-device prefix suggestions via `actor AutocompleteService` (lazy-load bundled JSON → sorted-array binary search, no per-keystroke network) + Apple-native `.searchable + .searchSuggestions + .searchCompletion` UI + zero-match fallback row + `RecentSearches` MainActor service (UserDefaults-backed, one-time legacy `recentSearches` → `barkain.recentSearches` key migration) + `scripts/generate_autocomplete_vocab.py` (offline Amazon autocomplete sweep: 702 prefixes × 2 sources, throttled, retry, electronics filter, ~5k terms cap, resume cache) + bundled `Barkain/Resources/autocomplete_vocab.json`. **Behavior change:** removed 300 ms auto-search-on-debounce; search now fires only on suggestion-tap or return-key submit (Apple-standard pattern) | +23 (`test_generate_autocomplete_vocab.py` — incl. 1 opt-in `BARKAIN_RUN_NETWORK_TESTS=1` smoke skipped by default) | +17 SearchViewModel +10 AutocompleteService +7 RecentSearches/+1 UI | #34 |
| 3d-noise-filter | Search cascade noise filter: live testing of 3d showed Tier 2 (Best Buy) returning only accessory/service rows for many flagship queries — "samsung flip 7" → cases + signage; "iphone 17 pro" / "macbook pro m4" / "airpods 4" → only AppleCare; "pixel 10" → Mobile Pixels monitors (brand collision); "switch 2" → games-for-Switch; "playstation 5 pro" → gift cards + Best Buy Protection. Old gate `if force_gemini or (not bestbuy_rows and not upcitemdb_rows)` skipped Gemini whenever Tier 2 returned anything, even pure noise. New `_is_tier2_noise(row)` classifier flags rows whose `category` contains `case / warrant / applecare / subscription / gift card / specialty gift / protection / monitor / physical video game / service / digital signage / charger / screen protector` OR whose `device_name` contains `applecare / protection plan / best buy protection / gift card / warranty / subscription / membership card / belt clip / skin case`. Cascade now escalates to Gemini when *relevant* Tier 2 count is 0; on escalation, noise rows are dropped from merge so they don't crowd out Gemini's flagship hits at `max_results`. Cost guard: real ASUS RTX 5090 listings keep cascade quiet for "rtx 5090" (no Gemini call). 9/9 probe queries fixed | +4 (`test_product_search.py`: cases-only / AppleCare-only / mixed-keeps-cost-guard / pixel-collision) | — | (this PR) |

**Test totals:** **335 + 6 + 29 + 26 + 42 + 10 + 14 + 26 + 23 + 4 backend** (3d-noise-filter adds 4 in `tests/modules/test_product_search.py`; 3d adds 23 in `tests/scripts/test_generate_autocomplete_vocab.py` + 1 opt-in smoke gated on `BARKAIN_RUN_NETWORK_TESTS=1`; six pre-existing auth tests in `test_auth.py`/`test_integration.py` fail in clean envs without DEMO_MODE setup, unrelated) + **100 iOS unit** + **4 iOS UI**.
...
2. **Phase 3 — Recommendation Intelligence (IN PROGRESS):** 3a ✅ text search (#22, #23). 3b ✅ eBay Browse API + deletion webhook (#24). 3c ✅ Search v2 (cascade + variant collapse + deep search + eBay affiliate fix, PR #32). 3c-hardening ✅ Live-test follow-on bundle (PR #32). 3d ✅ Autocomplete (#34). 3d-noise-filter ✅ Search cascade noise filter — Tier 2 noise classifier escalates to Gemini when only cases/AppleCare/protection/etc. surface (this PR). Next: 3e M6 Recommendation Engine (Claude Sonnet synthesis), 3f card rewards, 3g portal stacking, 3h image scan, 3i receipts, 3j identity stacking, 3k savings dashboard, 3l coupons, 3m hardening + `v0.3.0`.
...
> - **Search Tier 2 noise filter escalates to Gemini** (3d-noise-filter, 2026-04-20). Live testing of 3d showed the cascade swallowing flagship queries because Tier 2 (Best Buy Products API) returned only accessory/service rows — categories `Cell Phone Cases`, `AppleCare Warranties`, `Portable Monitors` (Mobile Pixels collision on "pixel 10"), `Physical Video Games` ("switch 2" → games), `All Specialty Gift Cards`, `Best Buy Protection`, `Digital Signage` (Samsung 75" interactive display matched "samsung flip 7"). Old gate `if force_gemini or (not bestbuy_rows and not upcitemdb_rows)` only fired Gemini on TOTAL Tier 2 emptiness. Fix in `backend/modules/m1_product/search_service.py`: new `_is_tier2_noise(row)` classifier (category contains `case / warrant / applecare / subscription / gift card / specialty gift / protection / monitor / physical video game / service / digital signage / charger / screen protector` OR title contains `applecare / protection plan / best buy protection / gift card / warranty / subscription / membership card / belt clip / skin case`); cascade now escalates when `relevant_tier2 == []` instead of when raw count is 0. Critical second step: when escalation fires AND Gemini returns rows, drop noise rows from the merge — otherwise the 5 noise rows fill the response slots before Gemini's flagship hits get in. Cost guard preserved: Tier 2 rows that pass the filter (real ASUS RTX 5090 cards) keep cascade quiet. 9/9 probe queries fixed (samsung flip 7 / samsung z flip 7 / iphone 17 pro / macbook pro m4 / airpods 4 / pixel 10 / playstation 5 pro / switch 2 still resolve via Gemini; rtx 5090 still served from BBY). Suppression skipped on `force_gemini` (the user explicitly asked for both sources) and when Gemini returns empty (half-noisy answer beats none). 4 tests in `test_product_search.py`
