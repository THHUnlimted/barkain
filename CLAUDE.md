...
|----|----------|-------|-------|
| rotating-cat-q4 | **HIGH** (from 2026-10-01) | Q3 2026 now seeded; Q4 is not. Discover's Q4 categories are reported as **Amazon + Target** — both Barkain retailers — so from 2026-10-01 every Discover holder shopping them gets steered to a worse card. Commented Q4 block staged in `scripts/seed_rotating_categories.py`; confirm both issuers' lists (~September, official pages not aggregators), fill in Chase, reseed. `scripts/check_catalog_freshness.py` starts failing 2026-09-16 | Phase 3 |
| local-pg-shadow | LOW | Host Postgres binds `127.0.0.1:5432`, shadowing the `barkain-db` container's `*:5432`. Every seed script (`scripts/_db_url.py` → `localhost:5432`) hits the wrong DB and dies with `role "app" does not exist`. Tests unaffected — they use 5433. See `docs/LOOSE_ENDS.md` §P5 | Phase 3 |

> **Recently closed** (kept here as a marker for one cycle, then drop):
> **2026-08-01:**
> `freshness-checker-selfbreak` — the `catalog-freshness` job added by the loose-ends sweep was red on PR #100 for its own reasons, not the catalog's. `check_catalog_freshness.py` gets `ROTATING_CATEGORIES` by importing `scripts/seed_rotating_categories.py`, which imported SQLAlchemy at module level; the workflow deliberately runs no `pip install` (reading the seed file instead of the DB is the whole point), so the checker died with `ModuleNotFoundError` while the seeded data was 60 days from expiry. SQLAlchemy moved into `seed_rotating` / `main`, with `TYPE_CHECKING` carrying the `AsyncSession` annotation. Verified by running the checker in a fresh dep-free venv. +8 BE tests (823→831) in `tests/scripts/test_check_catalog_freshness.py`: two `ast`-based pins asserting both scripts' module-level imports are stdlib-only (confirmed to flag the pre-fix file), a subprocess end-to-end run, and the ok / warn-window / expired date branches via `--today`. Stale comment in `catalog-freshness.yml` claiming no `pull_request` trigger corrected — it has a path-filtered one, and reports under the context name `catalog-freshness`, never `test`. **Pattern:** the sweep's own process note said a green check is only evidence if you know which job produced it; the converse bites harder — a *red* check nobody can act on is how tripwires get muted. An alarm that reads its source without importing it (`check_env_example.py`'s `ast` approach) has no dependency surface to break; one that imports pays for every dep its target grows.
> **2026-07-30 (loose-ends sweep):**
> `env-example-drift` — all 63 `config.py` settings now present in `.env.example`, live if secret / commented-with-default otherwise (an inert line can't drift, which was the SP-5 objection). New `scripts/check_env_example.py` parses `config.py` via `ast` (no imports, no deps, runs in a bare checkout) and exits non-zero on drift in either direction.
> `2i-d-L4` — watchdog heal now fetches the retailer's live search page via `_fetch_page_html`, using `search_url_template` from `config.json` and reusing `walmart_http._build_proxy_url` / `_CHROME_HEADERS` rather than duplicating the `user-`/`-country-us`/`:7000` rules. Soft-fails to an explicit `[PAGE HTML UNAVAILABLE — …]` marker that tells Opus to lower confidence instead of passing error text off as markup. +6 BE tests (817→823), one of which pins the exact original shape (`page_html != error_details`).
> `sourcing-ruff-split` — `sourcing/pyproject.toml` mirrors `backend/`'s `[tool.ruff]` (py312, line-length 99, `extend-select = ["E741"]`). Deliberately no `[tool.pytest.ini_options]`: the sourcing suite has zero async tests and adding one would move rootdir. 291 sourcing tests unaffected.
> `rotating-cat-expiry` → superseded by `rotating-cat-q4`. Q3 2026 seeded for both cards (dev DB: 0 → 2 rows effective today). **Impact correction:** Q3's real categories (gas, EV charging, transit, live entertainment, flights, drugstores, United Way) intersect `_RETAILER_CATEGORY_TAGS` **not at all**, so the four expired weeks produced the same recommendation a correct table would have. Q2 was genuinely wrong; Q4 will be. Tripwires added: `scripts/check_catalog_freshness.py` (fails 14 days before expiry, reads the seed file so it needs no PG) + `rotating_categories_all_expired` warning in `card_service` (now partitions rows in Python instead of filtering in SQL, so it can tell "none on file" from "all lapsed"). **Pattern:** expiring data needs an expiry *alarm*, not an expiry *field* — and the alarm belongs in cron, not in the blocking suite, or the next date rollover just trains people to ignore red CI.
> **2026-05-01:**
