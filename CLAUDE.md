...
> **Last updated:** 2026-04-21 (v5.12 — Benefits Expansion follow-ups: BE-L1 `program_type='membership'` retired, BE-L2 per-retailer scope dedup, BE-L9 iOS scope badge + honest membership-fee phrasing)
...
| Benefits Expansion (+follow-ups) | +10 student-tech programs + Amazon Prime Young Adult (`scope='membership_fee'`), +4 `*_direct` (acer/asus/razer/logitech), `is_young_adult` axis (migration 0010) + vocab. 52→63 programs / 8→12 retailers / 9→10 eligibility types. iOS `scope?:` + `isYoungAdult`. Follow-ups: BE-L1 `program_type='membership'` retired; BE-L2 `_dedup_best_per_retailer_scope` (Apple mil wins over edu, brand overlaps collapse); BE-L9 iOS `scopeBadge` pill + scope-aware `savingsText` ("50% off fee" / "Free shipping") + `age_verification` label | +8 | +7 | #45 |

**Test totals:** 518 backend + 124 iOS unit + 6 iOS UI. `ruff check` clean. `xcodebuild` clean.
...
- **Benefits Expansion (+follow-ups).** +10 student-tech + Prime YA (`scope='membership_fee'` reuses 3f-hotfix skip-savings); +4 `*_direct` in `BRAND_SPECIFIC_RETAILERS` + category keywords; `is_young_adult` (migration 0010) + vocab; iOS `scope?:` + `isYoungAdult`. Follow-ups: Prime Student → `identity` (scope alone drives skip); `_dedup_best_per_retailer_scope` both paths, key=(retailer_id, scope), rank -savings/-discount_value/name — different scopes survive; iOS `scopeBadge` + scope-aware `savingsText`; `age_verification` label
