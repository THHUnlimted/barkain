...
> **Last updated:** 2026-04-23 (v5.25 — fix/search-resolve-perf-1: tiered `_merge()` (strong conf ≥ 0.55 outranks weak DB — fixes Switch OLED→Switch 2); parallel Gemini+UPCitemdb in resolve paths (P50 17→5s, 404 34→13s); `upcitemdb.py` HTTPStatusError split; `ProductSearchResponse.cascade_path` populated. Backend 585→589. Full entry in docs/CHANGELOG.md.)
...
| search-resolve-perf-1 | Tiered `_merge()` by confidence (fixes Switch OLED→Switch 2); parallel Gemini+UPCitemdb in resolve paths (P50 17→5s, 404 34→13s); `upcitemdb.py` HTTPStatusError split; `ProductSearchResponse.cascade_path` | +6 | — | #TBD |

**Test totals:** 589 backend + 179 iOS unit + 6 iOS UI (with experiment flags off — see L-Experiment-flags-default-off). `ruff check` clean. `xcodebuild` clean.
...
- **Search v2 cascade + perf (search-resolve-perf-1).** normalize → Redis → DB pg_trgm@0.3 → Tier 2 `gather(BBY, UPCitemdb)` → Tier 3 Gemini only when Tier 2 irrelevant OR `force_gemini`. **Tiered merge**: rows split into strong (conf ≥ `_STRONG_CONFIDENCE=0.55`) / weak; within each `DB > BBY > UPCitemdb > Gemini` tiebreaks (`gemini_first` uses old partition) → `_collapse_variants`. `resolve_from_search` + `_resolve_with_cross_validation` parallelize Gemini+UPCitemdb via `gather(return_exceptions=True)` — P50 17s → 5s, 404 path 34s → 13s. Gemini retry-on-null externalized to `_get_gemini_data_retry`; fires on Gemini null regardless of UPCitemdb (gate removed after `test_gemini_null_retry_then_success` broke). `upcitemdb.py` splits `HTTPStatusError` (log body, no trace) from generic. `ProductSearchResponse.cascade_path` populated for iOS p95 attribution. `?query=` override on `/prices/{id}/stream` swaps retailer query + per-container `product_name`. eBay affiliate = modern EPN params on item URL. `_is_tier2_noise` = strict-majority denylist + model-code verbatim gate; tiered merge exposed `noise-filter-L1` (follow-up)
