...
> **Last updated:** 2026-04-21 (v5.10 — 3f-hotfix: identity savings compute against program's own retailer price; migration 0009 adds `discount_programs.scope` so Prime Student no longer over-claims savings)
...
| 3f | Purchase Interstitial + Activation Reminder. `PurchaseInterstitialSheet` from hero CTA + row taps (card headline + baseline-1% delta + conditional activation block). `POST /affiliate/click` gains `activation_skipped` telemetry (migration 0008). M6 cache key `:v2` (card/identity bust). Pre-fixes: `BarePreviewAPIClient` + `scripts/_db_url.py` + `without_demo_mode` fixture (killed 6 pre-existing auth failures, 8-step carry-forward). Alternatives rail scroll-to | +4 | +9 | #42 |
| 3f-hotfix | Identity savings correctness. `estimated_savings` now per-retailer (was: global cheapest). Migration 0009 adds `discount_programs.scope`; Prime Student → `membership_fee` (no phantom product savings). M6 cache `:v4` | +3 | — | — |

**Test totals:** 510 backend (0 pre-existing failures) + 117 iOS unit + 6 iOS UI. `ruff check` clean. `xcodebuild` clean.

**Migrations:** 0001 (initial, 21 tables) → 0002 (price_history composite PK) → 0003 (is_government) → 0004 (card catalog unique index) → 0005 (portal bonus upsert + failure counter) → 0006 (`chk_subscription_tier` CHECK) → 0007 (pg_trgm + trgm GIN idx) → 0008 (`affiliate_clicks.metadata` JSONB) → 0009 (`discount_programs.scope` — product / membership_fee / shipping). Drift marker in `tests/conftest.py::_ensure_schema` now checks `discount_programs.scope`.
...
- **3e M6 Recommendation Engine — deterministic.** Zero LLM. `POST /api/v1/recommend` does `asyncio.gather` over Prices+Identity+Cards+Portals + pure Python (< 150 ms p95). `final = base − identity`; card + portal deferred rebates on post-identity price. Tie-break: `effective_cost`, then condition, then well-known retailer. Brand-direct callout at `*_direct` ≥15 %. Excludes inactive/drift retailers. 15 min cache. iOS hero gated on 3 settle flags (`streamClosed` + `identityLoaded` + `cardsLoaded`); fetch fail → silent nil
- **3f Purchase Interstitial — deterministic.** Reuses 3e rec + cards. `PurchaseInterstitialSheet` from hero CTA + row taps via `@State`. Activation ack optimistic + session-scoped. `POST /affiliate/click` gains `activation_skipped` in JSONB `metadata` (0008). M6 cache key `:c<sha1>:i<sha1>:v2`. Baseline 1%. Alternatives rail `scrollTo(retailerId)` + 400 ms pulse. Pre-fixes: `BarePreviewAPIClient`, `scripts/_db_url.py`, `without_demo_mode` (retired 8-step 6-test carry-forward)
- **3f-hotfix Identity savings correctness.** (1) `estimated_savings` uses the program's OWN retailer price (was: global `min(price)`); unscraped brand-direct retailers fall back to highest scraped price. (2) Migration 0009 `discount_programs.scope` ∈ `{product, membership_fee, shipping}` — Prime Student flipped to `membership_fee`, service skips `estimated_savings` when scope ≠ product. M6 cache `:v4`. 17-program audit in CHANGELOG
