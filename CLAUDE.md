...
> **Last updated:** 2026-04-23 (v5.24 — chore/profileview-snapshot-infra-smoke + cleanup: +6 `ProfileView` snapshot tests covering 3 missing `content` branches (loading/error/kitchen-sink) + 3 state permutations (pro-user/affiliate-stats/saved-location), +5 `.accessibilityIdentifier` on shared sections, `MockAPIClient.getIdentityProfileDelay` test seam. 4 a11y-grep walker variants all failed on iOS 26.4 — PNG diff is the only signal; dead helper deleted. Audited other Features/* — `ProfileView` is the only view w/ the 3g-B dual-branch shape (L-smoke-7). iOS unit 173 → 179. Full entry in docs/CHANGELOG.md.)
