...
> **Last updated:** 2026-04-21 (v5.11 — Benefits Expansion: +10 student-tech programs, +4 `*_direct` retailers, `is_young_adult` axis + Amazon Prime Young Adult (`scope='membership_fee'`). Migration 0010)
...
| 3f (+hotfix) | Purchase Interstitial + Activation Reminder. `PurchaseInterstitialSheet` from hero CTA + row taps (card headline + baseline-1% delta + conditional activation block). `POST /affiliate/click` gains `activation_skipped` telemetry (migration 0008). M6 cache `:v4`. Pre-fixes: `BarePreviewAPIClient` + `_db_url.py` + `without_demo_mode` fixture. 3f-hotfix: `estimated_savings` now per-retailer with highest-scraped-price fallback; migration 0009 adds `discount_programs.scope`; Prime Student → `membership_fee` (no phantom product savings) | +7 | +9 | #42 |
| Benefits Expansion | Tech-brand catalog expansion (students + 18–24). +10 student-tech programs + Amazon Prime Young Adult (`scope='membership_fee'`). +4 `*_direct` retailers (acer/asus/razer/logitech). New `is_young_adult` axis (migration 0010) + `young_adult` vocab. 52→63 programs, 8→12 retailers, 9→10 eligibility types. iOS adds `scope?:` + `isYoungAdult` | +5 | +1 | (this) |

**Test totals:** 515 backend + 118 iOS unit + 6 iOS UI. `ruff check` clean. `xcodebuild` clean.
...
- **3f Purchase Interstitial + hotfix.** Reuses 3e rec + cards. `PurchaseInterstitialSheet` from hero CTA + row taps. Activation ack optimistic + session-scoped. `POST /affiliate/click` gains `activation_skipped` (0008). Baseline 1%. Alternatives rail `scrollTo(retailerId)` + 400 ms pulse. Pre-fixes: `BarePreviewAPIClient`, `_db_url.py`, `without_demo_mode`. Hotfix: per-retailer `estimated_savings` w/ highest-scraped fallback; migration 0009 `discount_programs.scope` ∈ `{product, membership_fee, shipping}` skips savings math when ≠ product; Prime Student → `membership_fee`. M6 cache `:v4`
- **Benefits Expansion.** 10 student-tech programs + Amazon Prime Young Adult (`scope='membership_fee'`, reuses 3f-hotfix skip-savings). 4 new `*_direct` retailers wired into `BRAND_SPECIFIC_RETAILERS` + category keywords. `is_young_adult` axis (migration 0010) + `young_adult` vocab. iOS `EligibleDiscount.scope?:` optional-decode. 52→63 programs / 8→12 retailers / 9→10 eligibility types
