...
> **Last updated:** 2026-04-29 (v5.44 — Step 3o-C Gemini UPC system-instruction rewrite. Strips electronics-only framing from `UPC_LOOKUP_SYSTEM_INSTRUCTION` (3,489→4,310 chars, 6 mixed-vertical examples covering electronics + appliance + pet + tool + lawn equipment + electronics, 9-step skeleton + JSON contract preserved). `build_upc_retry_prompt` rewritten to "all retail categories"; `build_upc_lookup_prompt` byte-unchanged. New `backend/tests/ai/test_upc_lookup_prompt.py` (8 anti-condensation regression tests pinning `# DO NOT CONDENSE` markers, length ≥3,000, all 9 steps in order, all 6 examples, JSON contract, electronics-only-removal, retry rewrite, builder-unchanged). Mini-bench `scripts/bench_grounded_3o_c.py` ran 5-UPC mixed-vertical panel × old+new prompts = 10 grounded calls; pass criterion `no_electronics_regression=True`. Pre-Fix #1 confirmed Makita-as-MacBook misresolve fixed under new prompt (UPC `088381675681` now correctly resolves to "Makita 18V LXT Lithium-Ion Cordless 1/2-inch Driver-Drill (XFD10Z)"). Backend tests +8 (773→781 passed; 8 skipped). Closes the 3o-A + 3o-B + 3o-C category-expansion trilogy. Per-step detail in `docs/CHANGELOG.md`.)
...
**Test totals:** 781 backend passed + 8 skipped (789 collected) + 207 iOS unit + 6 iOS UI (with experiment flags off — see L-Experiment-flags-default-off). 3o-C added +8 backend tests (773→781 passed). 3o-B added +19 backend tests (754→773 passed). 3o-A swung the count by −3 collected (parametrize collapse: dropped 9 `test_electronics_filter` cases, added 6 new). `ruff check` clean. `xcodebuild` clean.
...
| 3o-C-L1-fabricated-upc-tap | MEDIUM | Tier 3 Gemini search rows ship a fabricated `primary_upc`; iOS `SearchViewModel.resolveTappedResult:399` calls `/resolve` on it, grounded Gemini hallucinates an unrelated product (e.g. "rustoleum paint" → tap row #2 → resolves to Tide Pods Laundry Detergent). Pre-existing since 3c, but 3o-C's category breadth makes the wrong answer more visibly off-vertical. **Fix options:** (A) backend strips `primary_upc` from Tier 3 Gemini rows in `m1_product/service.py`, forcing iOS to use `/resolve-from-search` by name; (B) iOS gates UPC path on `result.source != .gemini` | Phase 3 |
| rustoleum-ux-L1 | LOW | SearchView with text in field + previous PriceComparisonView in nav stack: typing without dispatching the deep-search fallback row leaves the prior product visible underneath, creating "I searched X but got Y" misattribution. Confirmed via sim repro | iOS |
| rustoleum-ux-L2 | LOW | Autocomplete prefix-matches against `Rust-oleum-...` (with hyphen) but normalizes user input retains hyphenless `rustoleum` — zero suggestions surface for the canonical brand spelling. Strip hyphens in `AutocompleteService` matching key | iOS |
