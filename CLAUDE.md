...
> **Last updated:** April 2026 (v4.6 — Step 2f M11 Billing / RevenueCat + feature gating complete)
...
**Test counts:** 266 backend (266 passed / 6 skipped, +14 in Step 2f: 14 m11_billing covering 8 webhook event types + 3 status + 2 rate-limit + 1 migration), 63 iOS unit (+10 in Step 2f: 8 FeatureGateServiceTests + 2 ScannerViewModelTests), 0 UI, 0 snapshot.
...
13. **Remaining pre-fixes (not blockers):**
    - **Redeploy EC2 containers (2b-val-L1):** run `scripts/ec2_deploy.sh` to sync `i-09ce25ed6df7a09b2` with the repo — currently hot-patched via `docker cp` for `amazon/extract.js`, `bestbuy/extract.js`, and `bestbuy/base-extract.sh`. Next stop+start without redeploy will revert. Post-deploy MD5 verification block now flags drift automatically.
14. **Phase 2 continues:** Step 2g (M12 Affiliate Router) or Step 2h (background workers). Deferred from Step 2f: RevenueCat dashboard configuration (Mike post-merge task — products, entitlement, offering, webhook secret, Customer Center config), real Clerk iOS auth (separate step), App Store / TestFlight (Phase 4), promo codes / family sharing (Phase 4-5), backend-tracked scan quota (acceptable @AppStorage MVP bypass vectors), `subscription_tier` enum constraint (TEXT column with no CHECK is sufficient).
