...
- **Snapshot tests for branched render paths:** any view with multiple top-level render branches (e.g. `ProfileView`'s 4 `content` branches: loading, error, empty-profile `ScrollView`, `profileSummary` `ScrollView`) gets a test per branch in `BarkainTests/Features/<feature>/…SnapshotTests.swift`; baselines beside the test under `__Snapshots__/`. Record with `RECORD_SNAPSHOTS=1` in the scheme env; CI runs without it. Intra-branch state permutations that materially change layout get their own test (precedent: `test_kitchenSinkProfile_rendersAllChipRows`). See `ProfileViewSnapshotTests` (6 tests, from chore/profileview-snapshot-infra + smoke followup). **Implementation note:** accessibility-identifier grep as a secondary smoke assertion was tried 4 times across both chores (full `UIAccessibilityContainer` recursion, UIView-only, `accessibilityElements` array, bounded-bridge probe) — all failed on iOS 26.4's SwiftUI hosting bridge. The PNG diff is the only regression signal. Identifiers stay in view code as XCUITest anchors + contract markers
...
2. **Phase 3:** 3a–3d-noise-filter ✅ (#22–#36), ui-refresh-v1/v2/v2-fix ✅ (#37–#40), 3e (#41), 3f (+hotfix) ✅ (#42, #44), Benefits Expansion ✅ (#45–#47), FB Marketplace location + resolver ✅ (#48, #49), experiment/tier2-ebay-search ✅ (#50, opt-in), fb-resolver-followups + postfix-1 ✅ (#51, #52), 3g-A + 3g-B + 3g-B-fix-1 ✅ (#53, #54, #55). Next: prod FB seed (Mike), pill→Profile cross-tab nav, eBay-Tier-2 graduation call, 3h Vision, 3i receipts, 3k savings, 3l coupons, 3m hardening + `v0.3.0`. 3j folded into 3e
...
- **Search v2 cascade.** normalize → Redis → DB pg_trgm@0.3 → Tier 2 `gather(BBY, UPCitemdb)` → Tier 3 Gemini only when Tier 2 irrelevant OR `force_gemini`; merge DB>BBY>UPCitemdb>Gemini → `_collapse_variants` (2+ variants → synthetic generic). `?query=` override on `/prices/{id}/stream` swaps retailer query + per-container `product_name`. eBay affiliate = modern EPN params on item URL (legacy `rover.ebay.com` was a 42-byte pixel). `_is_tier2_noise` = strict-majority denylist + model-code verbatim gate
...
- **3g-B portal monetization iOS.** M6 cache key extends to `:c<...>:i<...>:p<sha1(active_portals)>:v5` — toggling membership busts stale recs immediately (same class of bug as 3f's card-portfolio hash). Membership signal hashes the *active* set only (falsy entries dropped) so toggling a portal off + back on doesn't double-bust. Winner-only `portal_ctas` on `StackedPath` (alternatives stay `[]` to keep payload tight). `PortalMembershipPreferences` mirrors `LocationPreferences` shape (UserDefaults wrapper, nonisolated, open-ended `[String: Bool]` so future portals don't need a migration). Interstitial portal row renders ≤3 sorted CTAs; FTC disclosure ("Referral — Barkain earns a bonus if you sign up.") inline + per-CTA on SIGNUP_REFERRAL only (FTC requires co-located, not a separate sheet). `affiliate_clicks.metadata` extended via existing 0008 JSONB — no new migration; new `portal_event_type` validated at the boundary against `_VALID_PORTAL_EVENT_TYPES` so iOS typos hit 422. **Codable acronym pitfall:** `.convertFromSnakeCase` maps `portal_ctas` → `portalCtas` (lowercase `as`); StackedPath uses `portalCtas` to round-trip cleanly, the local-only `PurchaseInterstitialContext` keeps Swift-style `portalCTAs`. **`ProfileView` dual-branch pitfall (3g-B-fix-1):** the view has two `ScrollView` branches — one for empty-profile users and one for completed-profile users (`profileSummary`). Adding a section means patching BOTH; 3g-B shipped wired only into the first → completed-profile users saw zero toggles until #55 fixed it. When adding any future Profile section, grep for the section above it (e.g. `cardsSection`) and confirm it appears in both branches. Demo seed `seed_portal_bonuses_demo.py` deleted; `test_seed_portal_bonuses_demo_is_idempotent` removed
