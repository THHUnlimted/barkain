// Lowe's extraction — DOM eval for search results.
// Anchor selector: best-guess — tries multiple patterns since not fully tested.
// Primary: [data-testid="product-card"], fallback: .plp-card, [data-productid].
// URL pattern: /pd/ (product detail pages).
...
(() => {
  ...
  if (cards.length === 0) {
    cards = document.querySelectorAll('.plp-card');
  }
  if (cards.length === 0) {
    cards = document.querySelectorAll('[data-productid]');
  }
  if (cards.length === 0) {
  ...
  const unique = Array.from(cards).filter(el => {
    const link = el.querySelector('a[href*="/pd/"]') || el.querySelector('a[href]');
  ...
  const listings = unique.slice(0, MAX).map((el, i) => {
    ...
    // Title — try data-testid first, then heading elements
    const titleEl = el.querySelector('[data-testid="product-title"]')
      || el.querySelector('h3 a')
      || el.querySelector('h3')
      || el.querySelector('h2 a')
      || el.querySelector('[class*="product-title"], [class*="ProductTitle"]');
    ...
    // URL — Lowe's uses /pd/ for product detail pages
    const linkEl = el.querySelector('a[href*="/pd/"]') || el.querySelector('a[href]');
    ...
    // Image
    const imgEl = el.querySelector('img[src*="lowes"]')
      || el.querySelector('img[src*="lowe"]')
