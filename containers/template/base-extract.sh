#!/usr/bin/env bash
# base-extract.sh — 9-step extraction skeleton for agent-browser containers.
# Retailer-specific containers override placeholders or replace this file entirely.
#
# Usage: ./base-extract.sh <query> [max_listings]
#
# Placeholders (replaced per retailer):
#   __SITE_HOMEPAGE__       — e.g., https://www.walmart.com
#   __SEARCH_URL_TEMPLATE__ — e.g., https://www.walmart.com/search?q=
#   __ANCHOR_SELECTOR__     — e.g., [data-item-id]
...
# Build search URL
SEARCH_URL="__SEARCH_URL_TEMPLATE__$(echo "$QUERY" | sed 's/ /+/g')"
...
while [ $attempt -lt $RETRY_MAX ]; do
  ...
  # Step 3: Warm up on site homepage (establish cookies, look human)
  jitter 800 1500
  ab open "__SITE_HOMEPAGE__" || { log "Warm-up failed (attempt $attempt)"; continue; }
  ab wait --load networkidle 2>/dev/null || true
  jitter 1500 3000
  ab scroll down $((150 + RANDOM % 250)) 2>/dev/null || true

  # Step 4: Navigate to search/listing page
  ...
  # Step 6: Handle overlays/modals (retailer-specific — override in per-retailer script)
  # Example: ab eval 'document.querySelectorAll("[role=\"dialog\"]").forEach(d => d.style.display = "none")'
  ...
  # Create JS file from extract.js with MAX_LISTINGS injected
