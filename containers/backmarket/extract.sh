#!/usr/bin/env bash
# BackMarket extraction script — standard agent-browser DOM eval pattern.
# Refurbished marketplace — all listings are refurbished condition.
# No auth required — public search results.
...
# Site-specific URLs
SITE_HOMEPAGE="https://www.backmarket.com"
SEARCH_URL="https://www.backmarket.com/en-us/search?q=$(echo "$QUERY" | sed 's/ /%20/g')"
...
while [ $attempt -lt $RETRY_MAX ]; do
  ...
  # Step 3: Skip homepage warmup — BackMarket has no PerimeterX/DataDome
  # gate, direct navigation is safe and saves ~10s.
  jitter 200 400
  ...
  # Step 6: Handle overlays/modals (BackMarket: cookie consent — dismiss if present)
