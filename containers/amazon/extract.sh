#!/usr/bin/env bash
# Amazon extraction script — 9-step agent-browser DOM eval pattern.
# Anchor selector: [data-component-type="s-search-result"] with data-asin attribute.
...
while [ $attempt -lt $RETRY_MAX ]; do
  ...
  # Step 3: Warm up on Amazon homepage (establish cookies, look human)
  jitter 800 1500
  ab open "https://www.amazon.com" || { log "Warm-up failed (attempt $attempt)"; continue; }
  ...
  # Step 6: Handle overlays/modals (Amazon: none typically needed)
