#!/usr/bin/env bash
# Target extraction script — uses load wait strategy (NOT networkidle).
# Target's analytics pixels fire indefinitely and networkidle hangs forever.
# After load, wait for [data-test='product-grid'] selector.
#
# Anchor selector: [data-test="@web/site-top-of-funnel/ProductCardWrapper"]
...
# Build search URL
SEARCH_URL="https://www.target.com/s?searchTerm=$(echo "$QUERY" | sed 's/ /+/g')"
