#!/usr/bin/env bash
# Facebook Marketplace extraction script — DOM eval with login modal handling.
# No bot detection observed — most permissive of all tested sites.
...
# Build search URL
SEARCH_URL="https://www.facebook.com/marketplace/search?query=$(echo "$QUERY" | sed 's/ /+/g')"
...
while [ $attempt -lt $RETRY_MAX ]; do
  ...
  # Step 3: Warm up on Facebook Marketplace homepage
  jitter 800 1500
  ab open "https://www.facebook.com/marketplace" || { log "Warm-up failed (attempt $attempt)"; continue; }
  ...
  # Step 6: Handle login modal — HIDE with display:none (NEVER .remove())
  # The DOM is fully rendered behind the CSS overlay. Hiding the modal
  # lets JS eval access everything without breaking React's virtual DOM.
  ab eval 'document.querySelectorAll("[role=\"dialog\"]").forEach(d => d.style.display = "none"); document.body.style.overflow = "auto"' 2>/dev/null || true
  jitter 500 1000
