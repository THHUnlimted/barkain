...
| Product resolution (UPC → canonical) | ⬜ | 1 | H | Gemini API UPC lookup (primary, 4-6s, high accuracy, YC credits) → UPCitemdb API (backup, free 100/day) → PostgreSQL persistent + Redis cache (24hr TTL) | AI resolves barcode to product name/brand/category. Cached aggressively — most barcodes only looked up once |
