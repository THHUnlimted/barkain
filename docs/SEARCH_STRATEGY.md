...
### `bench/vendor-compare-2` (2026-04-27) — clean-catalog re-run; B → E MIGRATE recommended

Replaces v1's contaminated catalog with a dual-filter pre-validated 9-entry catalog (7 valid + 2 invalid pattern UPCs) via new `scripts/bench_prevalidate_v2.py`. **Filter 1**: UPCitemdb has a record AND brand contains expected token (retry-on-429 with 5/15/45/90s backoff + 1.5s inter-call sleep to avoid the trial tier's per-minute cap). **Filter 2**: Gemini A-config grounded probe agrees on brand + name + chip + display size (mirrors the bench's `validate()` gate). 32 candidates → 9 survivors. Filter 1 dropped 22 (mostly UPCitemdb trial-DB misses across non-Apple flagships); Filter 2 dropped 3 (Galaxy Buds R170N → "Goodcook can opener" canary catch, Dyson V11 + Logitech G502 → Gemini null on UPCs UPCitemdb knows). 270 calls (9×6×5), ~14 min wall, ~$5.5 spend. **No production code paths changed.** New `scripts/bench_data/test_upcs_v2.json` + `--catalog` CLI flag on `scripts/bench_vendor_compare.py` (default keeps v1 catalog so prior runs reproduce).

**Headline**: B (current prod) and E_serper_then_D **tie at 24/28 = 85.7% recall but fail on different UPCs**. B fails iPad Air 13" M2 by confidently returning "Flash Furniture Lincoln 4-Pack 30-in High Backless Silver Antique Finish Metal Barstool" 4/4 warm runs (same Flash Furniture answer v1's contaminated catalog showed). E fails Xbox Series X by returning null 4/4 warm runs even though Serper's organic top-5 contains "Microsoft Xbox Series X 1TB Video Game Console" verbatim 5×. Fresh manual repro of E's Xbox call shows the synthesis returns the correct name 2/5 times — non-deterministic synthesis-prompt-interpretation issue (model reads "if snippets insufficient return null" too cautiously), not a Serper coverage gap. A held 27/28 (96.4%) but A is no longer production. **B vs E head-to-head**: tied recall, E **27% faster on p50** (2152 vs 2930 ms), **31% faster on p90** (2767 vs 3991 ms), **28× cheaper per call** ($0.0014 vs $0.040), and **safer failure mode** (null vs confidently wrong barstool). Production's `cat-rel-1-L4` post-resolve gate would catch B's iPad-as-barstool but only at the cost of a wasted Gemini call + DB write + cache write per occurrence; E's null fails fast.
