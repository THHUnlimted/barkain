...
| 0004 | `CREATE UNIQUE INDEX IF NOT EXISTS idx_card_reward_programs_product ON card_reward_programs (card_issuer, card_product)` — Step 2f takes Alembic ownership of the unique index that `scripts/seed_card_catalog.py` previously lazy-created at runtime. Idempotent via `IF NOT EXISTS` so existing dev/prod DBs upgrade cleanly; `CardRewardProgram.__table_args__` mirrors the index so test DBs built via `Base.metadata.create_all` get it on fresh schemas. | 2f |
| 0005 | Worker constraints — (a) `CREATE UNIQUE INDEX IF NOT EXISTS idx_portal_bonuses_upsert ON portal_bonuses (portal_source, retailer_id)` so `workers/portal_rates.py` can upsert deterministically on `(portal_source, retailer_id)`; (b) `ALTER TABLE discount_programs ADD COLUMN consecutive_failures INTEGER NOT NULL DEFAULT 0` — the discount verification worker counts hard HTTP/network failures here and flips `is_active=False` after `DISCOUNT_VERIFICATION_FAILURE_THRESHOLD` (default 3) consecutive failures. Both definitions are mirrored on the `PortalBonus.__table_args__` / `DiscountProgram` model so `Base.metadata.create_all` produces the same schema. | 2h |
| 0006 | `chk_subscription_tier` CHECK constraint — `ALTER TABLE users ADD CONSTRAINT chk_subscription_tier CHECK (subscription_tier IN ('free', 'pro'))`. Until 2i-b the column was unconstrained `TEXT` with a `'free'` server default; only `BillingService` writes to it but the DB had no defense against bad values. Idempotent via a `DO $$ ... END $$` block keyed on `pg_constraint.conname` so existing dev DBs upgrade cleanly. Mirrored on `User.__table_args__` in `app/core_models.py` so `Base.metadata.create_all` (test DB) gets it without alembic — same parity pattern as 0004 / 0005. | 2i-b |
| 0007 | pg_trgm extension + `idx_products_name_trgm` GIN index — `CREATE EXTENSION IF NOT EXISTS pg_trgm` followed by `CREATE INDEX idx_products_name_trgm ON products USING gin (name gin_trgm_ops)`. Backs the 3a text-search fuzzy matcher (`similarity(name, :q) > 0.3`). Idempotent via `DO $$ ... END $$` keyed on `pg_indexes.indexname`. `downgrade()` drops the index but deliberately leaves the extension in place so future indexes can reuse it. Mirrored on `Product.__table_args__` in `modules/m1_product/models.py` so `Base.metadata.create_all` (test DB) produces the same schema. conftest drift marker (`backend/tests/conftest.py::_ensure_schema`) updated to probe for this index. | 3a |
| 0008 | `ALTER TABLE affiliate_clicks ADD COLUMN IF NOT EXISTS metadata JSONB NOT NULL DEFAULT '{}'::jsonb` — Step 3f Purchase Interstitial uses `metadata.activation_skipped` to record whether the user acknowledged the rotating-bonus activation prompt before the browser hand-off. Mirrored on `AffiliateClick.__table_args__`. Downgrade drops the column. | 3f |
| 0009 | `ALTER TABLE discount_programs ADD COLUMN IF NOT EXISTS scope TEXT NOT NULL DEFAULT 'product'` + `chk_discount_programs_scope CHECK (scope IN ('product', 'membership_fee', 'shipping'))` — 3f-hotfix introduces scope so `IdentityService._build` skips estimated-savings math for non-`product` scopes (Prime Student's 50 % is off the Prime fee, not the product). Mirrored on `DiscountProgram.__table_args__`. conftest drift marker updated to probe `discount_programs.scope`. | 3f-hotfix |
| 0010 | `ALTER TABLE user_discount_profiles ADD COLUMN IF NOT EXISTS is_young_adult BOOLEAN NOT NULL DEFAULT false` — Benefits Expansion adds the 18–24 eligibility axis used by Amazon Prime Young Adult and any future age-based brand programs. Mirrored on `UserDiscountProfile` model. conftest drift marker updated to probe `user_discount_profiles.is_young_adult`. | Benefits Expansion |
...
CREATE TABLE user_discount_profiles (
    ...
    is_young_adult      BOOLEAN DEFAULT false,    -- added via migration 0010 (Benefits Expansion, 18–24 axis)
