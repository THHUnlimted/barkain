...
| fix/3o-C-L1-token-overlap-gate | Closes Known Issue `3o-C-L1-fabricated-upc-tap`. `_resolved_matches_query` (post-resolve relevance gate at `backend/modules/m1_product/service.py:67`) gains a 3rd check: token-overlap floor. When query has ≥2 meaningful tokens (`_meaningful_query_tokens` from `search_service.py`: ≥3 chars, not in `_RELEVANCE_STOPWORDS`), require ≥2 to substring-match the resolved haystack. Defends against in-brand cross-category drift the brand+strict-spec gates miss together — `Apple Watch Ultra 2 49mm Natural Titanium GPS Cellular` → grounded Gemini returned a real (not fabricated) MacBook Air UPC `195949036323`; `apple` matched both haystacks (gate 1 PASSES) and the strict-spec gate had no anchor (no voltage, no 4+digit pure-numeric — `49mm` doesn't satisfy `^\d{4,}$`); the new gate sees 7 meaningful tokens with only `apple` hitting → 1 < 2 → REJECT. Falls back to gates 1+2 only when query has <2 meaningful tokens (`iPhone 16 Pro` → `[iphone]` after stopword + length filter) so single-iconic-name resolves aren't penalized. Reuses `_meaningful_query_tokens` so search-time and resolve-time tokenize identically. Backend 815 → 817 (+2 tests pinning the Apple Watch → MacBook Air drift case + the iPhone 16 Pro short-iconic-query passthrough). Live-verified on a running backend: same query — (a) cache-hit branch invalidates the bad cached UPC and 404s with `gemini_reasoning` in the envelope; (b) fresh-upstream branch (Gemini returned no UPC + UPCitemdb rate-limited) falls cleanly to provisional → 4 real Apple Watch prices (Walmart pre-owned $341.96, eBay used $367.99, eBay open-box $419.95, FB used $650) in M2 + 3 misc rows in M14. Issue ID kept stable for grep-history; mechanism is "wrong-product UPC", not "fabricated UPC" — the UPC is a real GS1 entry. PR TBD | ✅ |
