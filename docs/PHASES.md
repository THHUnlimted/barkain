...
> Last updated: 2026-04-24 (v3.9 — demo-prep-1 (#63) F&F reliability pack lands: `/recommend` 422 + `/resolve` 404 explicit UX states, low-confidence 409 gate + `/confirm` endpoint + `ConfirmationPromptView`, `make demo-check` + `make demo-warm` CLIs. Backfilled search-resolve-perf-1 (#61) and search-relevance-1 (#62) rows that were missing from the table)
>
...
| bench/vendor-migrate-1 | Synthesis-prompt hardening (change "if snippets insufficient return null" → "if 3+ snippets clearly name the same product, return it") + small re-bench (5 UPCs × 5 runs = 25 calls, ~$0.05) to confirm Xbox 4/4 without AirPods regression → add E-with-fallback-to-B in production (when E returns null, fire B; cost-blended ~$0.0074/call avg) → swap `gather(B, UPCitemdb)` → `gather(E-with-fallback, UPCitemdb)` in `m1_product/service.py`. Mike app-tests broadly across electronics/appliances/tools (categories the dual-filter excluded from vendor-compare-2's catalog). Risk: production code change with measurable user impact; rollback plan = revert the gather swap | ⬜ |
