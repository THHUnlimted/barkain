...
| bench/vendor-compare-2 | Follow-up to vendor-compare-1: programmatically validate every catalog UPC against UPCitemdb's real index before adding it (accept entries only when the response brand+name matches the intended labels) AND verify Gemini's grounded resolution agrees with that label (cross-check observed during feat/grounded-low-thinking — UPCs aren't globally unique cross-brand, so UPCitemdb-validated alone is insufficient), re-run the same 6 configs against the cleaned catalog, ship the actual MIGRATE / STAY / PARTIAL recommendation. Estimated cost: ~$5-15. Tracks Known Issue `bench-cat-1` | ⬜ |
