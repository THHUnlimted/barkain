...
| 2i-c | Hardening: operational validation (LocalStack workers end-to-end — caught + fixed latent FK metadata bug in `scripts/run_worker.py` and `run_watchdog.py`), `conftest.py` schema drift auto-recreate via `chk_subscription_tier` marker, CI ruff enforcement. XCUITest target deferred to Phase 3. Tag `v0.2.0` is a Mike action post-merge. | ✅ (2026-04-15) |
