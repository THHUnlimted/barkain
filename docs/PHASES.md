...
> Last updated: 2026-04-22 (v3.6 — Phase 3 through Step 3g-A; Portal Live Integration backend slice; migration 0012; iOS half deferred to 3g-B)
>
...
| 3g-B | **Portal Live Integration — iOS slice.** `PortalCTA` model + interstitial portal row (≤3 sorted by rate desc; FTC disclosure conditional; signup promo amber line). Recommendation cache-key bump `:v4`→`:v5:pm<membership_hash>` so toggling membership busts stale recs. `affiliate_clicks.metadata.portal_used` extension (no migration — JSONB already shipped 0008). Profile → "The Kennel" portal-membership toggles. `seed_portal_bonuses_demo.py` deleted (worker now drives portal_bonuses) | ⬜ |
