...
| 2e | M5 Card Portfolio: card catalog seeding (top 30 cards from CARD_REWARDS.md), user card selection, rotating_categories seeding (current + next quarter) | ⬜ |
| 2f | M11 Billing: StoreKit 2 via RevenueCat, free/pro tier gating, feature flags | ⬜ |
| 2g | M12 Affiliate Router: link construction (Amazon Associates tag, eBay Partner Network campaign ID, CJ tracking), attribution logging | ⬜ |
