...
> Last updated: 2026-04-22 (v2.10 — fb-marketplace-location: **529 backend** / **138 iOS unit** / 6 iOS UI. **0 pre-existing failures**. Backend +9: `test_container_extract_request_validates_fb_location_slug` + `test_container_extract_request_validates_fb_radius_miles` + `test_extract_forwards_location_to_fb_marketplace_only` + `test_cache_key_includes_location_suffix_when_slug_set` (in `test_container_client.py`); `test_stream_forwards_fb_location_to_every_retailer_fake` + `test_stream_location_writes_scoped_cache_key` + `test_stream_location_different_slugs_use_different_cache_buckets` + `test_stream_endpoint_accepts_location_query_params` + `test_stream_endpoint_422_on_bad_location_slug` (in `test_m2_prices_stream.py`). iOS +14: new `LocationPreferencesTests.swift` (7) + new `EndpointsLocationTests.swift` (5) + 2 new in `ScannerViewModelTests.swift`. Drift marker unchanged: no schema changes.)
