...
> Last updated: 2026-04-22 (v2.11 — fb-marketplace-location-resolver: **557 backend** / **147 iOS unit** / 6 iOS UI. **0 pre-existing failures**. Backend +28: new `test_fb_location_resolver.py` covers normalization (abbreviations / suffixes / non-ASCII), canonical-name extraction, L1 Redis hit, L2 Postgres hit → L1 warm, live resolve + persist, engine fallback, tombstone on unresolved, 5-min Redis bar on throttled (no PG), singleflight contention (pub/sub notify invariant: subscribe-before-recheck), router happy/422/401. Existing `test_container_client.py` + `test_m2_prices_stream.py` fb-related tests rewritten for `fb_location_id` + `fb_radius_km` (miles→km at adapter). iOS +9 net: `LocationPreferencesTests.swift` rewritten (8 — drops slugify, adds v2 storage key + bigint-safe JSON encoding + nil-coord round-trip); `EndpointsLocationTests.swift` rewritten (7 — id-based params, large-bigint round-trip, `resolveFbLocation` path/body); new `LocationPickerViewModelTests.swift` (7 — state machine, save gating, clear reset); `ScannerViewModelTests.swift` updated for id. Drift marker bumped: `fb_marketplace_locations` table presence replaces `user_discount_profiles.is_young_adult` column probe.)
