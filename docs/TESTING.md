...
> Last updated: 2026-04-22 (v2.12 — fb-resolver-followups + extractor post-fix: **568 backend** / **156 iOS unit** / 6 iOS UI. **0 pre-existing failures** (with experiment/tier2-ebay flags off — defaults). Backend +11 vs v2.11: fb-resolver-followups added 4 (`test_resolve_endpoint_collapses_engine_to_live`, `test_resolve_endpoint_rate_limit_fires_on_sixth_call`, two stream `test_stream_fb_marketplace_*` for the `location_default_used` flag) + happy-path assertion on `resolution_path`; fb-resolver-postfix-1 added 6 (`test_parse_result_html_picks_canonical_city_over_subregion`, `test_parse_result_html_returns_none_when_only_subregion_validated`, `test_parse_result_html_falls_back_to_id_when_no_canonical_text`, `test_parse_result_html_normalizes_st_paul`, `test_parse_result_html_legacy_no_validation_when_arg_omitted`, `test_resolve_skips_engine_returning_only_subregion_hits`). iOS +9 vs v2.11: `EndpointsLocationTests.test_resolvedFbLocation_decodesResolutionPathFromSnakeCase` (1); `RecommendationDecodingTests.test_retailerPrice_decodes{LocationDefaultUsedTrue,LocationDefaultUsedAbsentDecodesAsNil}` (2) + new `makeRetailerPriceDecoder()` helper with `.iso8601`; `LocationPickerViewModelTests` retry trio + canonical-redirect trio (6). No new fixtures. No new migrations. Drift marker unchanged (`fb_marketplace_locations` from 0011). Pre-existing failure note: when `SEARCH_TIER2_USE_EBAY=true` is set in `backend/.env`, 11 tests in `test_product_search.py` fail — they assume UPCitemdb defaults; running with experiment flags off restores green.)
