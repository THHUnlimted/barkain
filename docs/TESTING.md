...
> Last updated: 2026-04-23 (v2.16 — chore/profileview-snapshot-infra-smoke followup: **585 backend** / **176 iOS unit** / 6 iOS UI. iOS +3 in `ProfileViewSnapshotTests` for the remaining `content` branches — `test_loadingBranch_rendersLoadingState`, `test_errorBranch_rendersEmptyState`, `test_kitchenSinkProfile_rendersAllChipRows`. `MockAPIClient` gains `getIdentityProfileDelay` so the loading branch is reachable mid-`.task`. ProfileView gains 5 `.accessibilityIdentifier` modifiers on shared sections (`kennelHeader`/`scentTrailsCard`/`subscriptionSection`/`marketplaceLocationSection`/`cardsSection`) — kept for future XCUITest coverage and as a self-documenting contract even after three walker variants of an accessibility-tree grep test all failed on iOS 26.4's SwiftUI hosting bridge (see §Snapshot Testing note). **Runner flag unchanged:** `-parallel-testing-enabled NO` still required. **Pre-existing failure mode** (carries from PR #50): 12 `test_product_search.py::test_search_tier2_*` failures fire when `SEARCH_TIER2_USE_EBAY=true` is in `backend/.env`; trunk default is False — override the flag for clean runs.)
