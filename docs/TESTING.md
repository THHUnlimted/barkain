...
> Last updated: April 2026 (v2.5 — 306 backend / 66 iOS unit / 2 iOS UI; identity discount product-relevance filter +4 tests)
...
| Step 2i-d addendum (2026-04-15) | 306 | 66 | 2 UI | 0 | **+4 backend** — identity discount product-relevance filter in `test_m5_identity.py`: `test_brand_direct_filtered_by_product_brand` (Apple.com shows for Apple product, Samsung.com hidden), `test_universal_retailer_always_shown` (Amazon Prime Student appears for any brand), `test_no_filter_without_product_id` (browse mode returns all), `test_category_filtered_programs` (Home Depot with `applies_to_categories=["Appliances","Tools","Home Improvement"]` hidden for Electronics, shown for Appliances). 3 existing savings tests updated to pass correct brand for `samsung_direct` programs. |
| **Total** | **306** | **66** | **2 UI** | **0** | (Phase 2 closed at end of Step 2i-d) |
