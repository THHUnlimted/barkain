...
> Last updated: 2026-04-23 (v2.17 — chore/profileview-snapshot-infra-smoke followup: **585 backend** / **179 iOS unit** / 6 iOS UI. iOS +6 in `ProfileViewSnapshotTests` across two axes — 3 missing `content` branches (`test_loadingBranch_rendersLoadingState`, `test_errorBranch_rendersEmptyState`, `test_kitchenSinkProfile_rendersAllChipRows`) and 3 shared-section state permutations (`test_proUserState_rendersProBadgeAndManageLink`, `test_nonZeroAffiliateStats_rendersTopTrail`, `test_savedMarketplaceLocation_rendersLocationLabel`). `MockAPIClient` gains `getIdentityProfileDelay` so the loading branch is reachable mid-`.task`. Pro-tier test exploits the DEBUG-empty-apiKey branch of `SubscriptionService.configure` as the only test seam that flips `currentTier` to `.pro`. ProfileView gains 5 `.accessibilityIdentifier` modifiers on shared sections (`kennelHeader`/`scentTrailsCard`/`subscriptionSection`/`marketplaceLocationSection`/`cardsSection`) — kept for future XCUITest coverage even after four walker variants of an accessibility-tree grep test all failed on iOS 26.4's SwiftUI hosting bridge (see §Snapshot Testing note). **Runner flag unchanged:** `-parallel-testing-enabled NO` still required. **Pre-existing failure mode** (carries from PR #50): 12 `test_product_search.py::test_search_tier2_*` failures fire when `SEARCH_TIER2_USE_EBAY=true` is in `backend/.env`; trunk default is False — override the flag for clean runs.)
...
- **No accessibility-grep assertion.** Three walker variants were tried across the original chore and the smoke followup — original (full `UIAccessibilityContainer` recursion), UIView-subview-only, and bounded-bridge-probe with wall-clock budget. All failed: the original wedged for 60+ seconds on iOS 26.4; the subview-only variants returned 0 identifiers because SwiftUI's `.accessibilityIdentifier` modifier doesn't propagate to `UIView.accessibilityIdentifier` nor to the `accessibilityElements` array on the rendered hosting-controller tree (only through the informal `UIAccessibilityContainer` protocol whose recursion is the original slow path). Identifiers on the 6 shared `ProfileView` sections (`kennelHeader`/`scentTrailsCard`/`subscriptionSection`/`marketplaceLocationSection`/`cardsSection`/`portalMembershipsSection`) are still added in view code — they're useful for XCUITest queries and serve as a self-documenting contract for "this section lives in both `ScrollView` branches."
- **Record workflow:**
